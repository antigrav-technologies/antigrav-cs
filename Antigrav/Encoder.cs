...
namespace Antigrav {
    internal class Encoder {
        ...
        private static readonly Regex ESCAPE_ASCII = new("([\\\\\"]|[^\\ -~])");
        private static readonly Regex HAS_UTF8 = new("[\\x80-\\xff]");
        ...
        private static string EncodeStringASCII(string s) {
            static string replace(Match match) {
                ...
                if (ESCAPE_DICT.ContainsKey(c)) return ESCAPE_DICT[c];
...
#pragma warning disable CS1718 // Выполнено сравнение с той же переменной
                ...
                string _encode_list(List<object> list, uint _current_indent_level) {
                    ...
                    if (indent != null) {
                        ...
                        separator = itemSeparator + _newline_indent;
                        buf += _newline_indent;
                    }
                    bool first = true;
